...
# -----------------------------
: "${BEGIN_PERIOD:=0}"
: "${DATA_DIR:=/home/haris/raid0/shared/haris/mydata_20260127/concat_daily_factor}"
: "${DEVICE:=cuda:0}"
: "${END_DATE:=}"
: "${EPOCHS:=80}"
...
: "${MODEL_TYPE:=mlp}"
: "${MODEL_SAVE_DIR:=/home/haris/mymodel_noon/models/StockPredictor_20260302}"
: "${NUM_PERIODS:=}"
: "${PREDICT_BATCH_SIZE:=0}"
: "${PREDICT_PERIOD_DAYS:=0}"
: "${PREDICTIONS_SAVE_DIR:=/home/haris/mymodel_noon/predictions/StockPredictor_20260302}"
: "${PROJECT_NAME:=StockPredictor_20260302}"
: "${MODEL_SAVE_FREQUENCY:=1}"
: "${SLIDE_PERIOD_DAYS:=0}"
: "${START_DATE:=20260106}"
: "${SUFFIX:=_20260302}"
: "${TRADE_DATE_PATH:=/home/haris/raid0/shared/haris/mydata_20260127/trade_date.fea}"
: "${TRAIN_BATCH_SIZE:=1}"
...
# -----------------------------
/home/haris/miniconda3/envs/myenv/bin/python /home/haris/mymodel_noon/merge_data_20260302_all_stocks.py

# -----------------------------
# Finish message
# -----------------------------
echo "Neural Network predictor pipeline finished!"
# cat /home/haris/logs/update_predict_20260302.log | mail -s "每日预测任务日志(noon) - $(date +\%Y-\%m-\%d)" xsheng9867@163.com