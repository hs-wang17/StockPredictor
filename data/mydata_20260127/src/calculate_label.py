...
stk_ret_20 = (all_stk_close_1300_df.pct_change(20).shift(-20)).dropna(how="all")
idx_ret_20_list, dates_list = [], []
for file in tqdm(MIN_DATA_DIR_LIST[:-20]):
    date = file.replace(".fea", "")
    stk_ret = stk_ret_20.loc[date].dropna()
    ...
    idx_ret_20_list.append(idx_ret)
    dates_list.append(date)
idx_ret_20 = pd.Series(idx_ret_20_list, index=dates_list)
...
label_20.to_feather("/home/haris/raid0/shared/haris/mydata_20260127/label.fea")
trade_date_df = pd.DataFrame({"trade_date": label_20.index})
trade_date_df.reset_index(drop=True).to_feather("/home/haris/raid0/shared/haris/mydata_20260127/trade_date.fea")
